                        ...
                        <div className="max-w-4xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-8">
                            <div className="text-center lg:text-left">
                                ...
                                <p className="text-gray-400 text-sm lg:text-base">
                                    Get weekly insights on Italian real estate trends and opportunities.
                                </p>
                            </div>
                            <form onSubmit={handleNewsletterSubmit} className="flex w-full max-w-md">
                                <div className="relative flex-1">
                                    <input
                                        ...
                                        className="w-full px-5 py-4 bg-gray-800 border border-gray-700 rounded-l-xl text-white placeholder-gray-500 focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 transition-all"
                                        disabled={subscribed}
                                    />
                                </div>
                                <button
                                    type="submit"
                                    disabled={subscribed}
                                    className={`px-6 py-4 font-bold rounded-r-xl transition-all duration-300 ${subscribed
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8 lg:gap-12">
                        ...
                        <div className="col-span-2">
                            <Link to="/" className="flex items-center space-x-3 mb-6">
                                ...
                                <div>
                                    <h2 className="text-lg font-black">SafeSquare</h2>
                                    <p className="text-xs text-gray-500 font-medium">Italian Real Estate Intel</p>
                                </div>
                            </Link>
                            <p className="text-gray-400 text-sm leading-relaxed mb-6 max-w-xs">
                                The first intelligence platform combining market value, demographics,
                                and risk data for definitive investment insights across 7,900+ Italian municipalities.
                            </p>
                            {/* Social Links */}
                            ...
                            <ul className="space-y-3">
                                {footerLinks.platform.map((link) => (
                                    <li key={link.label}>
                                        <Link
                                            to={link.path}
                                            className={`text-sm transition-colors ${link.disabled
                                        ...
                                        >
                                            {link.label}
                                            {link.disabled && (
                                                <span className="ml-2 text-[10px] bg-gray-800 px-1.5 py-0.5 rounded text-gray-500">
                                                    Soon
                                                </span>
                                            )}
                                        </Link>
                        ...
                        <div className="col-span-2 md:col-span-4 lg:col-span-2">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {dataSources.map((source) => (
                                    <div
                                        key={source.name}
                                        className="bg-gray-800/50 rounded-xl px-3 py-2 text-center hover:bg-gray-800 transition-colors"
                                    >
                                        <div className="text-sm font-bold text-white">{source.name}</div>
                                        <div className="text-[10px] text-gray-500">{source.desc}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="container mx-auto px-4 py-6">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                            <p className="text-sm text-gray-500">
                                © {currentYear} SafeSquare. All rights reserved.
                            </p>
                            ...
                            <p className="text-xs text-gray-600">
                                Made with ♥ for Italian Real Estate
                            </p>
                        </div>
                    </div>
                </div>
