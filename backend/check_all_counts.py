...
def check_counts():
    ...
    try:
        ...
        print(f"Investment Scores: {db.query(InvestmentScore).count()}")
        
        # Check coordinates
        coords_count = db.query(Municipality).filter(Municipality.centroid != None).count()
        print(f"Municipalities with coordinates: {coords_count}/{db.query(Municipality).count()}")
